...
public:
    ...
    void queryEfficient(int test)
    {
        ...
        if(levelA+levelB==maxDistance && maxASide>levelA)
        {
            for(int i=0;i<combination.size();i++)
            {
                if(combination[i].to1-1>levelA)
                {
                    ...
                    {
                        if(labels[levelARecord[j]][combination[i].lm1]==combination[i].to1-levelA) {
                            queueA[endA++] = levelARecord[j];
                            addi.push(levelARecord[j]);
                        }
                    }
                    while(!addi.empty())
                    {
        ...
        if(levelA+levelB==maxDistance && maxBSide > levelB)
        {
            for(int i=0;i<combination.size();i++)
            {
                if(combination[i].to2-1>levelB)
                {
                    ...
                    {
                        if(labels[levelBRecord[j]][combination[i].lm2]==combination[i].to2-levelB) {
                            queueB[endB++] = levelBRecord[j];
                            addi.push(levelBRecord[j]);
                        }
                    }
                    while(!addi.empty())
                    {
